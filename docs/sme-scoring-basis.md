...
| **A-04** | Prescriptive Feedback (PENDING VALIDATION — not yet confirmed against real SLMs, may still change) | **Checklist count.** Distinct feedback/intervention mechanism TYPES found anywhere in the document, classified into a fixed 4-type list (answer key, rubric, remediation referral, positive reinforcement); real content required, not a bare heading. Counts types, not instances, same reasoning as A-02. Revised from an earlier per-assessment coverage ratio, which collapsed to ~0% on nearly every real SLM since feedback is rarely attached per-assessment. | 3–4 / 2 / 1 / 0 types |
