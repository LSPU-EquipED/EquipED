...
conversion scripts rather than duplicated.
"""
...
def build_llm_rubric_guidance_config(
    guidance: str, bands: dict[int, str]
...
# criterion_code -> {guidance, bands: {score: descriptor}, scoring_rule summary}
# Covers SME's full 10 criteria; Coordinator's conversion uses all but A-05
# (Coordinator's A-05 is the unrelated curriculum_alignment strategy).
OP_A_QUALITATIVE_REWRITES: dict[str, dict[str, Any]] = {
    "OP-01": {
...
def run_qualitative_conversion(
    session: Any,
    agent_id: str,
    rewrites: dict[str, dict[str, Any]],
) -> None:
    """Convert `rewrites`' criteria to llm_rubric_guidance for one agent.

    Creates a draft from the agent's active rubric, updates each named
    criterion's strategy_config/scoring_rule, then publishes and
    activates the draft as the new live revision. Any criterion in the
    active rubric NOT present in `rewrites` (e.g. Coordinator's A-05) is
    left untouched. Raises if the draft is missing any criterion code
    that `rewrites` names.
    """
    ...
    for criterion in criteria:
        rewrite = rewrites.get(criterion.criterion_code)
        if rewrite is None:
            continue
        strategy_config = build_llm_rubric_guidance_config(
            rewrite["guidance"], rewrite["bands"]
        )
        update_criterion(
            session,
    ...
    if updated != len(rewrites):
        found = {c.criterion_code for c in criteria}
        missing = set(rewrites) - found
        raise RuntimeError(
            f"Expected to convert {len(rewrites)} criteria, converted "
            f"{updated}. Missing from draft: {sorted(missing)}"
        )
    ...
    session.commit()
    logger.info(
        "Published and activated %s rubric v%d (%s); active revision now %s",
        agent_id,
...
__all__ = [
    "OP_A_QUALITATIVE_REWRITES",
    "build_llm_rubric_guidance_config",
    "run_qualitative_conversion",
]
