            ...
            {documentTextGroups.length > 1 ? (
              <>
                <select
                  ...
                  className="bg-transparent text-sm font-bold text-slate-900 outline-none cursor-pointer focus:ring-0"
                >
                ...
                <span className="shrink-0 inline-flex items-center rounded-sm bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600 border border-slate-200">
                  {selectedAgentLabel}
                </span>
              </>
            ) : (
              <span className="inline-flex items-center gap-2 text-xs font-bold text-slate-700 uppercase tracking-wider select-none">
                <span className="shrink-0 inline-flex items-center rounded-sm bg-[#1b3b87]/5 text-[#1b3b87] px-2.5 py-1 text-[10px] font-extrabold border border-[#1b3b87]/15 uppercase tracking-widest">
                  {selectedAgentLabel}
                </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`inline-flex h-8 items-center gap-1.5 rounded-sm border px-3 text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer ${
                showFlagIndex
                  ? 'border-[#1b3b87] bg-[#1b3b87]/5 text-[#1b3b87]'
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        {isLoading || isResolvingEval || submitIsPending ? (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/70">
            <div className="flex items-center gap-3 rounded-sm border border-slate-200 bg-white px-5 py-4 text-xs font-bold text-slate-600 uppercase tracking-wider shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 flex flex-col items-center">
          {!!error && (
            <div className="w-full max-w-2xl rounded-sm border border-[#b91c1c]/30 bg-[#b91c1c]/5 px-4 py-3 text-sm text-[#b91c1c] font-semibold mb-6">
          ...
          {!isLoading && !error && activeGroup && (
            <div className="w-full max-w-2xl">
              ...
              {currentPage === (availablePages[0] || 1) && generalFlags.length > 0 && (
                <div className="mb-6 rounded-sm border border-[#f2c811]/30 bg-[#f2c811]/5 p-4">
                  <div className="flex items-center gap-2 text-xs font-bold text-[#1e293b] uppercase tracking-wider">
                    <AlertTriangle className="size-4 text-[#f2c811]" />
                  ...
                  <div className="mt-3 space-y-3">
                    {generalFlags.map((flag) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="rounded-full bg-[#f2c811]/15 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-[#1e293b] border border-[#f2c811]/30">
                        ...
                        {flag.justification && (
                          <p className="mt-1.5 text-xs leading-relaxed text-slate-600 bg-white p-2 border border-slate-100 rounded-sm">
              ...
              <div className="relative border border-slate-200 bg-white p-8 md:p-12 shadow-none rounded-sm min-h-[600px] flex flex-col justify-between">
                {/* Ledger Sheet Framing Details */}
                <div className="absolute inset-x-0 top-0 h-1.5 bg-[#1b3b87]" />
                <div className="absolute inset-x-8 top-3 flex items-center justify-between border-b border-slate-100 pb-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest select-none">
                ...
                {/* Page Content Chunks */}
                <article className="mt-6 flex-1 space-y-8 text-sm leading-[1.6] text-slate-700">
                      ...
                      return (
                        <section
                          ...
                          className={`rounded-sm transition-all duration-300 ${
                            hasFlags
                              ? 'border-l-4 border-l-[#f2c811] bg-[#f2c811]/3 px-4 py-3 space-y-4'
                              : 'border-l-4 border-l-transparent px-4 py-1'
                          }`}
                        >
                          {/* Chunk Text Content */}
                          <div className="space-y-3 font-normal text-slate-800">
                          ...
                          {hasFlags && (
                            <div className="mt-4 pt-3 border-t border-[#f2c811]/20 space-y-3">
                              {chunkFlags.map((flag) => (
                                <div
                                  key={flag.flag_id}
                                  className="rounded-sm border border-[#f2c811]/30 bg-white p-3 shadow-none"
                                >
                                  <div className="flex items-center gap-2">
                                    <Flag className="size-3.5 text-[#f2c811] fill-current" />
                                    <span className="rounded-full bg-[#f2c811]/15 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-[#1e293b] border border-[#f2c811]/40">
                                      Score {formatScore(flag.score)}/4
                                    </span>
                                  ...
                                  <h4 className="mt-2 text-xs font-bold leading-normal text-slate-900">
                                    {flag.criterion_text}
                                  </h4>
                                  {flag.justification && (
                                    <p className="mt-1.5 text-xs leading-[1.5] text-slate-600 bg-slate-50 p-2 border border-slate-100 rounded-sm font-medium">
                                      {cleanJustification(flag.justification)}
                                    </p>
                                  )}
                                </div>
                ...
                <div className="mt-12 border-t border-slate-100 pt-3 flex items-center justify-between text-[9px] font-bold text-slate-400 uppercase tracking-widest select-none">
          ...
          {!isLoading && !error && !activeGroup && (
            <div className="w-full max-w-2xl rounded-sm border border-slate-200 bg-white p-8 text-center text-slate-500">
        ...
        {showFlagIndex && (
          <div className="w-80 border-l border-slate-200 bg-white flex flex-col flex-shrink-0 animate-in slide-in-from-right duration-200">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 bg-slate-50">
              ...
              <button
                ...
                className="text-xs font-semibold text-slate-500 hover:text-slate-900 uppercase cursor-pointer"
              >
                Close
              </button>
            </div>
                ...
                selectedFlags.map((flag) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left rounded-sm border border-slate-200 bg-white p-3 transition-colors hover:border-[#1b3b87] hover:bg-slate-50/50 group focus:outline-none focus:ring-1 focus:ring-[#1b3b87] cursor-pointer"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5">
                          <Flag className="size-3 text-[#f2c811] fill-current" />
                          <span className="rounded-full bg-[#f2c811]/15 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-[#1e293b]">
                            Score {formatScore(flag.score)}/4
                          </span>
                        </div>
                        {linkedChunk && (
                          <span className="text-[10px] font-bold text-slate-400 group-hover:text-[#1b3b87]">
                            Page {linkedChunk.pageNumber}
                          </span>
                        )}
                      </div>
                      <p className="mt-2 text-xs font-bold leading-normal text-slate-800 line-clamp-2">
                        {flag.criterion_text}
                      </p>
                      {flag.justification && (
