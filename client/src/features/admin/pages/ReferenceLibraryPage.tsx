...
import type { ReferenceLibraryItem } from '@/shared/types/documents';
import {
  useDeleteReference,
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500 font-medium">
            Manage shared syllabus and curriculum references used by evaluations.
          </p>
        </div>
  ...
  return (
    <tr className="hover:bg-slate-50/50">
      <td className="py-3 px-4 align-top">
        <p className="text-sm font-semibold text-slate-900 truncate max-w-[16rem]" title={item.title}>
          {item.title}
        </p>
        {item.courseTitle ? (
      ...
      <td className="py-3 px-4 align-top text-right">
        <div className="flex items-center justify-end gap-2">
          ...
          >
            {isRebuilding ? <Loader2 className="size-3.5 animate-spin" /> : <RefreshCw className="size-3.5" />}
            Rebuild
          </button>
          <button
            type="button"
            ...
            title="Delete reference and all associated data"
          >
    ...
    >
      <div className="w-full max-w-md border border-slate-200 bg-white p-6 rounded-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 id="delete-reference-title" className="text-base font-bold text-slate-900">
              Delete reference?
            </h3>
            <p className="mt-1 text-sm font-medium text-slate-600">
              This will permanently remove <span className="font-semibold text-slate-900">{item.title}</span> and
              all stored chunks, embeddings, and the local PDF file.
            </p>
            {item.embeddingReady ? (
