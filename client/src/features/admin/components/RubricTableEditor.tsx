...
export function RubricTableEditor() {
  ...
  const getTablesForAgent = (agentId: AgentRubricId) => tables.filter((table) => table.agentId === agentId);
  ...
  const updateTableTitle = (tableId: string, title: string) => {
    setTables((current) => current.map((table) => (table.id === tableId ? { ...table, title } : table)));
  };
  ...
  return (
    <section className="grid gap-5">
      <div>
        <div className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">Admin</div>
        ...
        {agents.map((rubric) => {
          ...
          >
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button type="button" variant="outline" className="ml-auto gap-2" onClick={() => addTable(rubric.id)}>
            ...
            {agentTables.map((table) => {
              ...
              <div key={table.id} className="grid gap-2 rounded-lg border p-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button type="button" variant="outline" className="gap-2" onClick={() => addRow(rubric.id, table.id)}>
                    <Plus className="size-4" aria-hidden="true" />
                    Add row
                  </Button>
            ...
            {agentTables.length === 0 && (
              <div className="flex min-h-24 items-center justify-center rounded-lg border border-dashed">
                <Button type="button" variant="outline" className="gap-2" onClick={() => addTable(rubric.id)}>
                  <Plus className="size-4" aria-hidden="true" />
                  Add table
                </Button>
              </div>
