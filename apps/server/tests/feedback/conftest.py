...
def _make_evaluation_job(db_session, *, owner_id):
    ...
    db_session.add(
        CriterionScore(
            ...
            document_id=document_id,
            criterion_id="OP-01",
            criterion_title="Organization and Presentation",
            score=3,
            justification="Adequate organization and presentation.",
        )
    )

    gad_result = AgentResult(
        evaluation_id=job.evaluation_id,
        document_id=document_id,
        agent_name="gad",
        subtotal=2.0,
    ...
    db_session.add(gad_result)
    db_session.flush()

    db_session.add(
        CriterionScore(
            agent_result_id=gad_result.agent_result_id,
            evaluation_id=job.evaluation_id,
            document_id=document_id,
            criterion_id="GAD-01",
            criterion_title="Free from gender stereotypes",
            score=2,
            justification="Some stereotyped representations observed.",
        )
    )
