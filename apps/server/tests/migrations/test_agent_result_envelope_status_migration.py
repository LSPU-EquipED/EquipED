"""Tests for 20260915_0001_add_agent_result_envelope_status migration."""
...
def test_migration_upgrade_and_downgrade(tmp_path):
    url = f"sqlite+pysqlite:///{tmp_path / 'test_envelope_status.db'}"
    engine = create_engine(url)

    # Prepare schema up to 20260911_0001
    with engine.begin() as conn:
        conn.execute(
            text("CREATE TABLE alembic_version (version_num VARCHAR(32) PRIMARY KEY)")
        )
        conn.execute(text("INSERT INTO alembic_version VALUES ('20260911_0001')"))
        conn.execute(
            text(
    ...
    # Downgrade back to 20260911_0001
