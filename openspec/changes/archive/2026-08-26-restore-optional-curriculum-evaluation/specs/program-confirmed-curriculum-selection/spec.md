...
**Reason**: Partial-only launch was an over-broad retirement behavior; the product requires an optional full path for supported programs.

**Migration**: Replaced by `Faculty chooses explicit full or partial evaluation intent` below. Historical jobs retain their persisted intent.
