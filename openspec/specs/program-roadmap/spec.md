# Program Roadmap Spec

## Purpose

Structured per-program career roadmap data (course placement by year/semester, tech stacks, competency stages) stored as versioned institutional reference data. Roadmaps give the evaluation pipeline program-level context that document retrieval cannot provide reliably for table-heavy roadmap documents. Roadmap source PDFs are provenance-only: never embedded, never in the reference library.
