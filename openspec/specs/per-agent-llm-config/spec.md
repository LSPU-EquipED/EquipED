# per-agent-llm-config Specification

## Purpose
Define per-agent LLM model configuration for parallel evaluation execution, enabling each agent to use a distinct model to avoid rate-limit contention.
