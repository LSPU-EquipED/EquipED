...
## Purpose
Restricts institutional document uploads (syllabus, curriculum, policy, rubric) to admin users only and clarifies shared access for syllabus/curriculum references. Policy management and access remain admin-only with no-existence-leak.
...
#### Scenario: Policy existence is not leaked to faculty
...
#### Scenario: Admin cross-admin policy management
