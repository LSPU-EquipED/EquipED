...
The system SHALL make explicitly confirmed no-curriculum partial evaluation the only new faculty evaluation path. It SHALL require confirmed program context and an acknowledgement before accepting the request.

#### Scenario: Faculty confirms a new partial evaluation
- **WHEN** a faculty user confirms a program and acknowledges the no-curriculum warning for their processed SLM
...
The system SHALL visibly mark new no-curriculum results partial and SHALL skip Coordinator before dispatch. Historical completed evaluations SHALL preserve their original partial/full state even if a retired curriculum link is cleared.
